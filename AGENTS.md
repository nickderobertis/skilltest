...
| `pyproject.toml`, `uv.lock` | The Python side's workspace root: `[tool.uv.workspace]` over `sdks/python` + `plugins/pytest`, and the single lockfile both resolve into. Virtual root — nothing is published from here. |
